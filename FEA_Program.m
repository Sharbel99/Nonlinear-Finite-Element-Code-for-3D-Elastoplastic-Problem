...
q11_plot    = zeros(1,NbLoadSteps+1);
q22_plot    = zeros(1,NbLoadSteps+1);
q12_plot    = zeros(1,NbLoadSteps+1);
q33_plot    = zeros(1,NbLoadSteps+1);
C1111_plot = zeros(1,NbLoadSteps+1);
C2222_plot = zeros(1,NbLoadSteps+1);
C1212_plot = zeros(1,NbLoadSteps+1);

F       = zeros(3,3,8,NbLoadSteps+1);
be     = zeros(3,3,8,NbLoadSteps+1);
    for li=1:8     
        F(1:3,1:3,li,1)  = eye(3);
        be(1:3,1:3,li,1)  = eye(3);
    end
q      = zeros(3,3,8,NbLoadSteps+1);
alpha = zeros(8,NbLoadSteps+1);
...
for iter=1:maxiterations
    ...
    fprintf('   %4d    |     %3d      |   %1.7e   |   %1.7e\n',...
            step,iter,R,R/R0);
    if R<R0.*tol || R<10^-15 || iter == maxiterations
...
dispplot(1:3,step+1)   = Node_U_V(3, 1:3);

strainplot(1:6,step+1) = strain(1:6,3);
stressplot(1:6,step+1) = stress(1:6,3);
q11_plot(1,step+1)        = q(1,1,4,step+1);
q22_plot(1,step+1)        = q(2,2,4,step+1);
q33_plot(1,step+1)        = q(3,3,4,step+1);
q12_plot(1,step+1)        = q(1,2,4,step+1);
C1111_plot(1,step+1)    = C_ep(1,1,4,step+1);
C2222_plot(1,step+1)    = C_ep(2,2,4,step+1);
C1212_plot(1,step+1)    = C_ep(4,4,4,step+1);
...
figure (2)
plot(dispplot(1,:),stressplot(1,:),'b','linewidth',3)
hold on
plot(dispplot(1,:),stressplot(2,:),':r','linewidth',3)
plot(dispplot(1,:),stressplot(4,:),'--g','linewidth',2)
plot(dispplot(1,:),stressplot(3,:),'k','linewidth',2)
hold off
grid on
xlabel ('\delta_1')
xlim ([0 1.3])
...
figure(4)
plot(t,q11_plot(:),'linewidth',2)
hold on
plot(t,q22_plot(:),'--','linewidth',2)
plot(t,q12_plot(:),'linewidth',2)
plot(t,q33_plot(:),'linewidth',2)
hold off
grid on
xlabel ('time (s)')
xlim ([0 1.2])
legend('q_{11}','q_{22}','q_{12}','q_{33}', 'location', 'northwest')

figure(5)
plot(t,C1111_plot(:),'linewidth',2)
hold on,
plot(t,C2222_plot(:),'--','linewidth',2)
plot(t,C1212_plot(:),'linewidth',2)
hold off
grid on
xlabel ('time (s)')
xlim ([0 1.2])
legend('C^{ep}_{1111}','C^{ep}_{2222}','C^{ep}_{1212}', 'location', 'best')
...
plotModel(NodeTable2, ix, numel, nen, 7, 1, 1, 'Deformed Configuration (2D)', 'y', 'y')
plotModel(NodeTable, ix, numel, nen, 7, 1, 1, 'Deformed Configuration (2D)', 'n', 'n')