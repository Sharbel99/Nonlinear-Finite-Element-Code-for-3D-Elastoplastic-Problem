  ...
  if ndm ==2
    ...
    switch iel
        case 1 %Small-Deformation Isotropic Elastostatics Element
            if ndm == 3
                [ElemK, ElemF,F(:,:,:,step+1),be(:,:,:,step+1),q(:,:,:,step+1),alpha(:,step+1),C_ep(:,:,:,step+1),stress,strain] = ...
                    Elast3d_Elem(xl,mateprop,nel,ndf,ul,F(1:3,1:3,1:8,step),be(1:3,1:3,1:8,step),q(1:3,1:3,1:8,step),alpha(1:8,step));
                %L_Elem1_3d
            else %ndm == 2
